...
class MediaController extends BaseController
{
    public function getMediaList(Request $request)
    {
        ...
        $medias = DB::table('lms_content_library as content_library')
            ...
            ->select(
                ...
                'content_library.content_types_id as contentTypeId',
            ...
            if($medias->count() > 0){
                foreach($medias as $media){
                    if ($media->mediaUrl != '') {
                        ...
                        } else if ($media->contentTypeId == 5 || $media->contentTypeId == 8) {
    ...
    public function getMediaById($contentId)
    {
        ...
        $contentLibrary = $contentLibrary->select('content_library.content_id as contentId','scorm.id as scormID','scorm.version as scormVersion', 'content_library.content_name as contentName', 'content_library.content_version as contentVersion', 'content_library.content_types_id as contentTypeId', 'content_type.content_type as contentType', 'content_library.media_id as mediaId', 'media.media_name as mediaName', 'media.media_url as mediaUrl', 'content_library.parent_content_id as parentContentId', 'parent_content_library.content_name as parentContentName', 'scormDetails.launch', 'content_library.is_active as isActive')->first();
